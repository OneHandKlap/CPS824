...
def generate_returns(episode, gamma=0.9):
    ...
    epi_returns=np.array([np.dot(gammas[:len(gammas)-i],rewards[i:]) for i in range(len_episode)])
...
def mc_policy_evaluation(env, policy, Q_value, n_visits, gamma=0.9):
    ...
    for i in range(len(episode)):
        ...
        if (visit_flag[state,action]==0):
            ...
            Q_value[state,action] = Q_value[state,action] + (1/n_visits[state,action])*(returns[i]-Q_value[state,action])
...
def epsilon_greedy_policy_improve(Q_value, nS, nA, epsilon):
    ...
    for s in range(nS):
        #get indices of actions that are the "best"
        actions=np.argwhere(Q_value[s] == np.argmax(Q_value[s])).flatten().tolist()

        num_best_actions=len(actions)
        for i in range(nA):
            if i in actions:
                new_policy[s,i]+=1-epsilon
...
def td_sarsa(env, iterations=1000, gamma=0.9, alpha=0.1):
    ...
    for i in range(1,iterations):
        ...
        for j in range(1,500):
            s_t2,r,done,_=env.step(a_t1)
            a_t2=sample_action(policy,s_t2)
            Q_value[s_t1,a_t1]=Q_value[s_t1,a_t1]+alpha*((r+gamma*(Q_value[s_t2,a_t2])-Q_value[s_t1,a_t1]))
...
def qlearning(env, iterations=1000, gamma=0.9, alpha=0.1):
    ...
    for i in range(1,iterations):
        ...
        for j in range(1,500):
            s_t2,r,done,_=env.step(a_t1)
            a_t2=sample_action(policy,s_t2)
            Q_value[s_t1,a_t1]=Q_value[s_t1,a_t1]+alpha*((r+gamma*np.max(Q_value[s_t2])-Q_value[s_t1,a_t1]))
...
if __name__ == "__main__":
    ...
    #     td_results_sto=[]
    #     ql_results_sto=[]
    #     indices=[x for x in range(0,10001,1000)]
    #     iterations
    ...
    #             env=gym.make("Deterministic-4x4-FrozenLake-v0")
    #             Q_mc, policy_mc = mc_glie(env, iterations=i, gamma=0.9)
    #             temp_mc_results_det.append(test_performance(env,policy_mc))
    #             Q_td, policy_td = td_sarsa(env, iterations=i, gamma=0.9, alpha=0.1)
    #             temp_td_results_det.append(test_performance(env,policy_td))
    #             Q_ql, policy_ql = qlearning(env, iterations=i, gamma=0.9, alpha=0.1)
    #             temp_ql_results_det.append(test_performance(env,policy_ql))
    #             env=gym.make("Stochastic-4x4-FrozenLake-v0")
    #             Q_mc, policy_mc = mc_glie(env, iterations=i, gamma=0.9)
    #             temp_mc_results_sto.append(test_performance(env,policy_mc))
    #             Q_td, policy_td = td_sarsa(env, iterations=i, gamma=0.9, alpha=0.1)
    #             temp_td_results_sto.append(test_performance(env,policy_td))
    #             Q_ql, policy_ql = qlearning(env, iterations=i, gamma=0.9, alpha=0.1)
    #             temp_ql_results_sto.append(test_performance(env,policy_ql))
            
    #         mc_results_det.append(np.mean(temp_mc_results_det))
    #         mc_results_sto.append(np.mean(temp_mc_results_sto))
    #         td_results_det.append(np.mean(temp_td_results_det))
    #         td_results_sto.append(np.mean(temp_td_results_sto))
    #         ql_results_det.append(np.mean(temp_ql_results_det))
    #         ql_results_sto.append(np.mean(temp_ql_results_sto))
    #     def make_fig(indices,results_det,results_sto,plt_title):
    #         fig, (ax1,ax2)=plt.subplots(2,1)
    #         fig.suptitle("Environment Comparison")
    #         ax1.plot(indices,results_det,'o-')
    #         ax1.set_ylabel("Success Rate for Deterministic")
    #         ax2.plot(indices,results_sto,'o-')
    #         ax2.set_ylim(0,100)
    #         ax2.set_ylabel("Success Rate for Stochastic ")
    #         plt.savefig(plt_title)
    #     make_fig(indices,mc_results_det,mc_results_sto,"mc_fig.png")
    #     make_fig(indices,td_results_det,td_results_sto,"td_fig.png")
    #     make_fig(indices,ql_results_det,ql_results_sto,"ql_fig.png")
